...
from shared.config import DEFAULT_LANGUAGE, GROQ_API_KEY, LLM_API_KEY, LLM_BASE_URL, LLM_MODEL, WHISPER_BACKEND
...
async def check_groq() -> dict | None:
    ...
    if not GROQ_API_KEY:
        return None
    from application.state import groq_limits

    if groq_limits:
        return dict(groq_limits)
    async with httpx.AsyncClient(timeout=10) as client:
        resp = await client.get(
